...
export default function ModulesPage() {
  ...
  const validateModuleName = (name: string): string | null => {
    if (!name) return null;
    if (/[A-Z]/.test(name)) return 'Must be lowercase only';
    ...
    if (/--/.test(name)) return 'Consecutive hyphens are not allowed';
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (validateModuleName(formData.name)) {
      setError(`Invalid module name: ${validateModuleName(formData.name)}`);
      return;
    }
    if (validateProviderName(formData.provider)) {
      setError(`Invalid provider name: ${validateProviderName(formData.provider)}`);
      return;
    }
      ...
      {modules.length === 0 ? (
        <div className="text-center py-12 bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700">
          ...
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
            Click "Add Module" to add your first Terraform/OpenTofu module from a Git repository
          </p>
        </div>
      ) : (
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        setNameError(validateModuleName(val));
                      }}
                      placeholder="e.g., vpc, eks, rds"
                      className={`w-full px-3 py-2 border rounded-md shadow-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-indigo-500 focus:border-indigo-500 ${nameError ? 'border-red-500 dark:border-red-500' : 'border-gray-300 dark:border-gray-600'}`}
                      required
                    />
                    <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
                      Used in <code className="font-mono">&lt;namespace&gt;/&lt;name&gt;/&lt;provider&gt;</code>
                    </p>
                    {nameError && (
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        setProviderError(validateProviderName(val));
                      }}
                      placeholder="e.g., aws, azure, gcp"
                      className={`w-full px-3 py-2 border rounded-md shadow-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-indigo-500 focus:border-indigo-500 ${providerError ? 'border-red-500 dark:border-red-500' : 'border-gray-300 dark:border-gray-600'}`}
                      required
                    />
                    <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
                      The Terraform/OpenTofu provider name
                    </p>
                    {providerError && (
                      <p className="mt-1 text-xs text-red-600 dark:text-red-400">{providerError}</p>
                    )}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    className={`w-full px-3 py-2 border rounded-md shadow-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-indigo-500 focus:border-indigo-500 ${gitUrlError ? 'border-red-500 dark:border-red-500' : 'border-gray-300 dark:border-gray-600'}`}
                    required
                  />
                  <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
