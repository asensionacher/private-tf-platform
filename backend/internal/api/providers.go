...
func GetProviders(c *gin.Context) {
	...
	query := `
		SELECT p.id, p.namespace_id, p.name, p.display_name, p.description, p.synced, p.created_at, p.updated_at, 
			   n.name as namespace
		FROM providers p
		JOIN namespaces n ON p.namespace_id = n.id
	`
	...
	for rows.Next() {
		...
		if err := rows.Scan(&p.ID, &p.NamespaceID, &p.Name, &p.DisplayName, &p.Description, &p.Synced,
...
func GetProvider(c *gin.Context) {
	...
	err := database.DB.QueryRow(`
		...
		JOIN namespaces n ON p.namespace_id = n.id
		WHERE p.id = $1
	`, id).Scan(&p.ID, &p.NamespaceID, &p.Name, &p.DisplayName, &p.Description, &p.Synced,
...
func CreateProviderFromGit(c *gin.Context) {
	...
	// Strip the "terraform-provider-" prefix if the user accidentally included it.
	// The registry protocol requires only the short name (e.g. "time", not "terraform-provider-time").
	// Keep the original as display_name for UI purposes.
	displayName := input.Name
	registryName := strings.TrimPrefix(input.Name, "terraform-provider-")
	...
	// Validate provider name conforms to Terraform naming rules:
	// lowercase letters, digits, and hyphens only; no leading/trailing/consecutive hyphens.
	if !isValidProviderName(registryName) {
		c.JSON(http.StatusBadRequest, gin.H{"error": "Invalid provider name. Must contain only lowercase letters, digits, and hyphens, and must not start or end with a hyphen (e.g., \"random\", \"aws\", \"my-provider\")"})
		return
	}
	...
	`, input.NamespaceID, registryName).Scan(&existingID)
	...
	_, err = database.DB.Exec(`
		INSERT INTO providers (id, namespace_id, name, display_name, description, source_url, synced, git_auth_type, git_auth_data, created_at, updated_at)
		VALUES ($1, $2, $3, $4, $5, $6, FALSE, $7, $8, $9, $10)
	`, providerID, input.NamespaceID, registryName, displayName, input.Description, input.GitURL, sql.NullString{String: "https", Valid: input.IsPrivate && input.GitUsername != ""}, authData, now, now)
...
func UploadProviderPlatform(c *gin.Context) {
	...
	// providerName may contain a slash (e.g. "hashicorp/random"); sanitize for use in a path
	safeProviderName := strings.ReplaceAll(providerName, "/", "-")
	outputDir := filepath.Join(buildDir, "providers", namespace, safeProviderName, version)
	...
	// Generate filename
	filename := "terraform-provider-" + safeProviderName + "_" + version + "_" + osParam + "_" + arch + ".zip"
